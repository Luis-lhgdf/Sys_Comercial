...
class AppearanceManager:
    def __init__(self):
        ...
        self.default_libtheme_location = pkg_resources.resource_filename(
            "customtkinter", "assets/themes/"
        )

        self.custom_theme_location = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "themes/custom_colors.json"
        )

        self.current_theme_location = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "themes/current_theme.json"
        )
        ...
        self.highlight_color = self.json_fgcolor(
            self.current_theme, "CTkButton", "hover_color"
        )
    ...
    def write_color_to_theme(self, theme_color):
        # Carrega o arquivo JSON do tema atual
        with open(self.current_theme_location, "r") as config_file:
            theme = json.load(config_file)

        # Define a nova cor no tema
        theme["theme"]["default_theme"][0] = theme_color

        # Escreve de volta no arquivo JSON
        with open(self.current_theme_location, "w") as config_file:
            json.dump(theme, config_file, indent=4)
