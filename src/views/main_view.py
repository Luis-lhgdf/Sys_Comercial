...
class MainView(ctk.CTk):
    ...
    def create_database_view(self):
        ...
        """
        self.warning_text_box = ctk.CTkTextbox(self.frame2, fg_color="transparent",
                                               font=self.appearance_manager.get_font_body(bold=True), width=500, height=310)
        self.warning_text_box.grid(column=0, row=3, padx=(10, 0), pady=(0, 40))


        self.warning_text_box.insert("0.0", warning_text)
        self.warning_text_box.configure(state="disabled", text_color="black", border_color="black", corner_radius=5, wrap="word", fg_color="#D9B46E")

        self.create_db_button = ctk.CTkButton(self.frame2, text="Criar", height=43, width=140,
                                            font=self.appearance_manager.get_font_subtitle(True), text_color="white",
                                            fg_color="#558FAD", hover_color="#D9B46E", command=self.create_database_dialog)
        self.create_db_button.grid(column=0, row=4, padx=(60, 0), pady=(5, 5), sticky="w")
