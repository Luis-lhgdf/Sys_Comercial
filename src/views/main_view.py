...
class MainView(ctk.CTk):
    ...
    def login_view(self):
       ...
       # Configuração das colunas principais
        ...
        self.sign_up_text = ctk.CTkLabel(self.frame2, text="Você não tem uma conta?", text_color="black", width=5, font=self.appearance_manager.get_font_body())
        ...
        self.welcome_label = ctk.CTkLabel(self.frame2, text="BEM-VINDO DE VOLTA", font=self.appearance_manager.get_font_title(),text_color="black")
        self.welcome_label.grid(column=0, row=2, padx=(60,0), pady=(0, 0), sticky="w")

        self.welcome2_label = ctk.CTkLabel(self.frame2, text="Acesse sua conta", font=self.appearance_manager.get_font_subtitle(),text_color="black")
        self.welcome2_label.grid(column=0, row=3,padx=(60,0),pady=(0, 40), sticky="w")

        self.login_text = ctk.CTkLabel(self.frame2, text="Login", font=self.appearance_manager.get_font_subtitle(),text_color="black")
        self.login_text.grid(column=0, row=4, padx=(65,0),pady=(5, 3), sticky="w")

        self.login_entry = ctk.CTkEntry(self.frame2, placeholder_text="Digite seu login",fg_color="white", text_color="black", placeholder_text_color="gray", height=43, width=300, border_color="gray80", font=self.appearance_manager.get_font_body())
        self.login_entry.grid(column=0, row=5, padx=(60,0),pady=(0, 5), sticky="w")


        self.password_text = ctk.CTkLabel(self.frame2, text="Senha", text_color="black", font=self.appearance_manager.get_font_subtitle())
        self.password_text.grid(column=0, row=6, padx=(65,0),pady=(5, 3), sticky="w")

        self.password_entry = ctk.CTkEntry(self.frame2, placeholder_text="Digite sua senha",fg_color="white", text_color="black", placeholder_text_color="gray", height=43, width=300, border_color="gray80", show="*", font=self.appearance_manager.get_font_body())
        self.password_entry.grid(column=0, row=7, padx=(60,0), pady=(0, 3), sticky="w")

        

        self.show_password_check = ctk.CTkCheckBox(self.frame2, text="", command=lambda:self.show_password(self.password_entry), height=43, fg_color="#D9B46E", hover_color="#D9B46E")
        ...
        self.database_status = ctk.CTkButton(self.frame2, image=alert_icon, anchor="w", text="", text_color="blue", fg_color="transparent",
                                              hover_color="#D9B46E", font=self.appearance_manager.get_font_body(), command=self.create_database_view)
        self.database_status.grid(column=0, row=10, pady=(50, 5), columnspan=3, sticky="s")

    def create_database_view(self):
        self.restart_interface(self.frame2)

        # Configuração das colunas e linhas do frame 2
        self.frame2.grid_columnconfigure(0, weight=1)
        self.frame2.grid_rowconfigure((1, 10), weight=1)

        self.back_button = ctk.CTkButton(self.frame2, text="", width=0, image=back_icon, fg_color="#558FAD", hover_color="#D9B46E", command=self.back)
        self.back_button.grid(column=0, row=0, padx=(10, 10), pady=(5, 0), sticky="w")


        self.create_db_label = ctk.CTkLabel(self.frame2, text="CRIAR NOVO BANCO DE DADOS", font=self.appearance_manager.get_font_title(), text_color="black")
        self.create_db_label.grid(column=0, row=2, padx=(60,0), pady=(0, 0), sticky="w")

        warning_text = """Por favor, defina uma senha para criar um novo banco\nde dados sqlite. Essa senha é necessária para \nacessar e proteger seus dados. Mantenha-a em um\nlocal seguro, pois não poderá ser recuperada ou \nalterada posteriormente. Obrigado."""

        self.warning_text_label = ctk.CTkLabel(self.frame2, text=warning_text, text_color="black",justify="left", anchor="w",fg_color="transparent", font=self.appearance_manager.get_font_subtitle())
        self.warning_text_label.grid(column=0, row=3,padx=(60,0),pady=(0, 40),)

        self.password_bd_text = ctk.CTkLabel(self.frame2, text="senha", text_color="black",font=self.appearance_manager.get_font_subtitle())
        self.password_bd_text.grid(column=0, row=4, padx=(65,0),pady=(5, 3), sticky="w")

        self.password_bd_entry = ctk.CTkEntry(self.frame2, placeholder_text="Digite sua senha",fg_color="white",show="*", text_color="black", placeholder_text_color="gray", height=43, width=300, border_color="gray80", font=self.appearance_manager.get_font_body())
        self.password_bd_entry.grid(column=0, row=5, padx=(60,0),pady=(0, 5), sticky="w")


        self.password_bd_confirmation_text = ctk.CTkLabel(self.frame2, text="Redigite sua senha",text_color="black", font=self.appearance_manager.get_font_subtitle())
        self.password_bd_confirmation_text.grid(column=0, row=6, padx=(65,0),pady=(5, 3), sticky="w")

        self.password_bd_confirmation = ctk.CTkEntry(self.frame2, placeholder_text="Redigite sua senha", fg_color="white", text_color="black", placeholder_text_color="gray", height=43, width=300, border_color="gray80", show="*", font=self.appearance_manager.get_font_body())
        self.password_bd_confirmation.grid(column=0, row=7, padx=(60,0), pady=(0, 3), sticky="w")


        self.show_password_check = ctk.CTkCheckBox(self.frame2, text="", command=lambda:self.show_password(self.password_bd_confirmation), height=43, fg_color="#D9B46E", hover_color="#D9B46E")
        self.show_password_check.grid(column=0, row=7, padx=(370, 0), pady=(5, 5), sticky="w")


        self.create_db = ctk.CTkButton(self.frame2, text="Criar",height=43, width=140,  font=self.appearance_manager.get_font_subtitle(True),text_color="white", fg_color="#558FAD", hover_color="#D9B46E")
        self.create_db.grid(column=0, row=9, padx=(60,0), pady=(5, 5), sticky="w")
   

        self.database_status = ctk.CTkLabel(self.frame2, anchor="w", text="Atualizar local do seu banco de dados",
                                            text_color="black", fg_color="transparent", font=self.appearance_manager.get_font_subtitle())
    
        self.database_status.grid(column=0, row=10, padx=(60,0), pady=(50, 5),  columnspan=3, sticky="w")

        
        self.select_database = ctk.CTkButton(self.frame2, image=view_icon, width=0, anchor="w", text="Procurar",height=43,  font=self.appearance_manager.get_font_subtitle(True),text_color="white", fg_color="#558FAD", hover_color="#D9B46E")
    ...
    def restart_interface(self, frame):
        # Destruir todos os widgets existentes
        for widget in frame.winfo_children():
            widget.destroy()

    def back(self):
        self.restart_interface(self)
        self.login_view()
