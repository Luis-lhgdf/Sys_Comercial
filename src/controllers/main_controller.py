...
class MainController:
    ...
    def validate_login(self):
        ...
        if self.model.validate_login_db(login_get, password_get):
            self.model.on_login_success(login_get, self.view.info_list_user)
            return True
        else:
    ...
    def create_database_and_set_path(self):
        db_path = filedialog.asksaveasfilename(
            ...
            initialfile="database_syscomercial.db",
        )
        if db_path:
            self.model.crate_database(database_path=db_path)
            self.view.create_db_button.configure(
                text=f"Banco de dados criado com Sucesso", command=None
            )
            self.login(self.model.db_connection())

    def search_database(self):
        db_path = filedialog.askopenfilename(
            defaultextension=".db", filetypes=[("Banco de Dados SQLite", "*.db")]
        )
    ...
    def validade_create_newuser(self, login):
        ...
        self.model.db_cursor.execute(
            ...
            (
                login,
            ),  # Certifique-se de adicionar uma vírgula após o login para torná-lo uma tupla de um elemento
        )
        ...
        if query_result:
            self.utils.msgbox(
                "Criar usuario",
                "Ja existe um usuario com este nome, por favor escolha outro",
                0,
            )
            return False
